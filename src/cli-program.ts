...
export function createProgram(version: string): Command {
  ...
  program
    ...
    .action(async (opts: { target: string; mode: string; dryRun?: boolean; force?: boolean; include?: string[]; exclude?: string[] }) => {
      await runAction({
        action: 'install',
  ...
  program
    ...
    .action(async (opts: { target: string; json?: boolean; verbose?: boolean }) => {
      await listResources({
        target: assertTarget(opts.target),
